...
export container_name=finance-redis-dev;
export project_name=finance-redis;
export image_name=ubuntu-redis;
export ip=${finance_admin_redis_ip};
...
mbt_rewrite;
devCreate() {
	if ! docker run -d --name $container_name --net aegis-bridge --ip $ip -p 6010:6379 $image_name > /dev/null; then
